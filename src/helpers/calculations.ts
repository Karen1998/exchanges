import { ICalculatedData, IChartData, IChartDataForStatistic } from "src/types/types";
...
const chartDataRef: IChartDataForStatistic[] = [];
let modDataRef: { [key: number]: number }[] = [];
const calculatedDataRef: ICalculatedData = {
    average: 0,
    standardDeviation: 0,
    averageDeviation: 0,
    mod: 0,
...
onmessage = (ev: MessageEvent<TDataEvent>): void => {
    ...
    let calculatedData = calculatedDataRef;
    ...
    postMessage('Done', JSON.stringify(chartDataRef));
}
